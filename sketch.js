...
function setup() {
	...
	text("Hummingbird Music Limited", padXmin+bleedPadW, padYmax*0.3)
	var fontSizeNorm = Math.round(cardH/14); 
	fill(51);
	textSize(60);
	text("      Tang Tsz-Kei", padXmin+bleedPadW, (padYmax*0.65)+fontSizeA*0);
	fill(0, 123, 123); 
	textSize(20);
	text("                         Occupation: Singer, song writer, actress", padXmin+bleedPadW, (padYmax*0.70)+fontSizeA*1);
	fill('rgb(0,255,0)'); 
	text("                     Tel: +852 2824 2270", padXmin+bleedPadW, (padYmax*0.70)+(fontSizeA*2));
	fill('rgba(100%,0%,100%,0.5)');
	text("                 FB: @G.E.M.OfficialPage", padXmin+bleedPadW, (padYmax*0.70)+(fontSizeA*3));
fill('rgba(100%,0%,100%,0.5)');
	text("        19/F., Catic Plaza,8 Causeway Road, Causeway Bay,HK", padXmin+bleedPadW, (padYmax*0.70)+(fontSizeA*4));
